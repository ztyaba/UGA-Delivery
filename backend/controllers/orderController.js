...
function ensureOrderAccess (order, user) {
  ...
  if (user.role === 'driver' && order.assignedDriver && order.assignedDriver.toString() === user.id) {
    return true;
  }
  if (user.role === 'driver' && !order.assignedDriver && order.status === 'Ready for Pickup') {
    return true;
  }
...
async function listOrders (req, res) {
  ...
  if (role === 'customer') {
    ...
    if (view === 'available') {
      ...
      const vendorIds = driver?.profile?.approvedVendors || [];
      filter = {
      ...
      filter.assignedDriver = id;
    }
  } else {
    return res.status(403).json({ message: 'Role not permitted to list orders' });
  }
